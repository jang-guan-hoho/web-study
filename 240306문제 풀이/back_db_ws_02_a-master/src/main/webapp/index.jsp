...
<%
    ...
    String message = welcomeMessageList[randomNumber];
%>
	<%-- 
		Q. welcomeMessages 배열을 출력하도록 아래 코드를 작성해주세요.
	 --%>
    <h1>랜덤 환영 메시지들 : <%= Arrays.toString(welcomeMessageList) %></h1>
	...
	<%
		for(int messageIdx = 0; messageIdx < welcomeMessageList.length; messageIdx++) {
			...
			if(welcomeMessageList[messageIdx].equals(message)) {
				...
				out.print("<p><b>" + welcomeMessageList[messageIdx] + "</b></p>");
				
			} else {
				
				out.print("<p>" + welcomeMessageList[messageIdx] + "</p>");
